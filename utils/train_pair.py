...
def train(data_path, models_path, backend, snapshot, batch_size, alpha, epochs, start_lr, gpu, scale, feat_loss,
          dataset, model_type):
    ...
    if dataset == 'camvid':
        ...
        # randomscale = (1.0,)
        train_ds = CityScapes(data_path, model_type=model_type, cropsize=cropsize, mode='train',
    ...
    if model_type == 'pspnet':
        ...
        if dataset == 'camvid':
            models = {
                'squeezenet': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=512,
                                             deep_features_size=256, backend='squeezenet'),
                'densenet': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=1024,
                                           deep_features_size=512, backend='densenet'),
                'resnet18': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=512,
                                           deep_features_size=256, backend='resnet18'),
                'resnet34': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=512,
                                           deep_features_size=256, backend='resnet34'),
                'resnet50': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=2048,
                                           deep_features_size=1024, backend='resnet50'),
                'resnet101': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=2048,
                                            deep_features_size=1024, backend='resnet101'),
                'resnet152': lambda: PSPNet(sizes=(1, 2, 3, 6), n_classes=class_num, psp_size=2048,
    ...
    if feat_loss:
        if dataset == 'camvid':
            if model_type == 'pspnet':
                ...
                highres_net, _ = build_network(models,
                                               "./exp/pspnet18-camvid/scale1.0_epoch100_pure/PSPNet_resnet18_1.0_92_.pth",
                                               backend)
            else:
                raise NotImplementedError
    ...
    for epoch in range(starting_epoch, starting_epoch + epochs):
        ...
        for x, y, y_cls in train_iterator:
            ...
            with torch.no_grad():
                if feat_loss:
                    if model_type == 'pspnet':
                        ...
                        downsample_highres_p = F.interpolate(highres_p,
                                                             [int(cropsize[1] * scale), int(cropsize[0] * scale)],
                                                             mode='bilinear', align_corners=True)
                    else:
                        raise NotImplementedError

            x = F.interpolate(x, [int(cropsize[1] * scale), int(cropsize[0] * scale)], mode='bilinear',
                              align_corners=True)
            ...
            if model_type == 'pspnet':
                ...
                seg_loss, seg_loss16, seg_loss32 = seg_criterion(out, y), seg_criterion(out_feat16, y), seg_criterion(
                    out_feat32, y)
        ...
        if mIOU > max_mIoU:
            ...
            torch.save(net.state_dict(),
                       os.path.join(models_path, '_'.join(["PSPNet", backend, str(scale), str(epoch + 1), '.pth'])))
