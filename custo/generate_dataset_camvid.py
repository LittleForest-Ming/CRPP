...
for key_dist in range(0, ref_gap):
    ...
    decoded_output_dir = os.path.join(camvid_sequence_root, "%dM-GOP%d/decoded_GOP%d_dist_%d/" % (
    int(bitrate / 1000), ref_gap, ref_gap, key_dist))
    MV_output_dir = os.path.join(camvid_sequence_root,
                                 "%dM-GOP%d/MVmap_GOP%d_dist_%d/" % (int(bitrate / 1000), ref_gap, ref_gap, key_dist))
    ...
    for split in ['train', 'val', 'test']:
        ...
        for scene in os.listdir(orig_dir):
            ...
            dataset_decoded_idx_gap = scene_length_info[scene]['dataset_start_idx'] - scene_length_info[scene][
                'encoded_start_idx']
            ...
            for idx, name in enumerate(image_list[begin_idx:]):
                ...
                encode_cmd = "ffmpeg -y -i %s" % workspace_dir + "/%03d.png -f rawvideo -pix_fmt yuv420p " + os.path.join(
                    workspace_dir, "proxy.yuv")
                ...
                cmd = "%s/x265 --input-res 960x720 --fps %d --rect --amp --input %s/proxy.yuv --bitrate %d --keyint %d --bframes 0 %s/proxy_%d.hevc" % (
                workspace_dir, encode_fps, workspace_dir, bitrate, GOP, workspace_dir, bitrate)
                os.system(cmd)
